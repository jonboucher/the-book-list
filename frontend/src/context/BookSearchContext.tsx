...
export const BookSearchProvider = ({ children }: BookSearchProviderProps) => {
  ...
  const searchBooks = async (query: string): Promise<void> => {
    const res = await fetch(`http://localhost:3000/search-books?search=${query}`);
    const data = await res.json();
    setSearchResults(data.items);
  };

  return <BookSearchContext.Provider value={{ searchResults, searchBooks }}>{children}</BookSearchContext.Provider>;
};
